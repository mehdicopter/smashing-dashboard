...
def find_bus_directions(destinations, stations, type, id)
  directions = {}
  ...
  destinations.each_index do |index|
    ...
    possible_directions.each_index do |index_direction|
      ...
      schedule = get_as_json("#{API_HOME}/schedules/#{type}/#{id}/#{stations[destination['name']]}/#{direction}?_format=json")
...
def read_timings(type, id, stop, dir)
  ...
  begin
    ...
    puts("ERROR: Unable to fetch timings for #{type} #{id} (#{url}): #{err}")
    return nil
  end
  ...
  if schedules.length >= 2
    ...
    puts("ERROR: Unable to parse timings for #{type} #{id} (#{url}), json = #{json}")
  end
end
